...
export default function FilterMenu({ filters, setFilters }: FilterMenuProps) {
  ...
  const handlePriceChange = (range: [number, number]) => {
    setFilters(f => ({ ...f, minPrice: String(range[0]), maxPrice: String(range[1]) }));
  };
  ...
  return (
    <aside className="w-64 bg-zinc-900 flex flex-col justify-between p-6 border-r border-zinc-800 shadow-lg">
      <div>
        ...
        <PriceRangeSlider value={[minPrice, maxPrice]} onChange={handlePriceChange} />
        ...
        <div className="mb-6">
          ...
          <select value={filters.condition} onChange={handleConditionChange} className="block w-full rounded-md bg-zinc-900 border border-zinc-600 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#fedc04]">
        ...
        <div className="mb-6">
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="block w-full rounded-md bg-zinc-900 border border-zinc-600 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#fedc04]">
            {sortOptions.map((option, i) => (
              <option key={option} value={sortOptionsValues[i]}>{option}</option>
            ))}
          </select>
        </div>
        {/* Apply Filters */}
        <div className="mt-4">
          <button onClick={handleApplyFilter} className="w-full py-2 px-4 bg-[#fedc04] text-black font-bold rounded-md hover:bg-yellow-400 transition">
            Apply Filters
          </button>
        </div>
      </div>
